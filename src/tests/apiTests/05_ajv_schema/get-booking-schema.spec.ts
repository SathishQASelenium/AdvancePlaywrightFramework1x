/**
 * JSON Schema validation with Ajv against the Restful Booker GET /booking/{id}
 * response. Schema lives in src/testdata/schemas/get-booking.schema.json.
 *
 * Three checks:
 *   1. A static sample payload validates (schema sanity).
 *   2. A LIVE GET /booking/{id} response validates (contract test).
...
const log = createLogger('schema-validation-get');

const schema = JSON.parse(
    fs.readFileSync(
        path.join(__dirname, '../../../testdata/schemas/get-booking.schema.json'),
        'utf-8',
    ),
);

test.describe('@P0 @regression Ajv schema validation - GET /booking/{id}', () => {
    test('validates the get-booking contract', async ({ bookingApi }, testInfo) => {
        ...
        // 2 — live response from the real API: create a booking then fetch it.
        await test.step('Live GET /booking/{id} response matches the schema', async () => {
            const created = await bookingApi.createBooking(buildBooking({ firstname: 'SchemaGet' }));
            const body = await bookingApi.getBooking(created.bookingid);
            const { valid, errorText } = validateSchema(schema, body);
            log.info(`Step 2: live booking ${created.bookingid} valid=${valid}`);
